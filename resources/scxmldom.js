...
class SCXMLDoc extends XMLDocument {
	createElementNS(nsURI, name, ...rest) {
		const el = super.createElementNS(nsURI, name, ...rest);
		if (nsURI===NS) {
			Object.setPrototypeOf(el, (nodeClassByName[name] || SCXMLCustom).prototype);
		} else {
			Object.setPrototypeOf(el, SCXMLCustom.prototype);
		}
		return el;
	}
...
class SCXMLTransition extends Element {
	// Add a new child script action
	// code: (optional) script code to add initially
	addScript(code='') {
...
class SCXMLScript extends Element {
	delete() {
...
class SCXMLCustom extends Element {
	// Delete this element
	delete() {
		const container = this.parentNode;
		this.remove();
...
Object.assign(SCXMLDoc.prototype, {
	StateClass: SCXMLState,
	TransitionClass: SCXMLTransition,
	ScriptClass: SCXMLScript
});
